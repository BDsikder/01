...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', function() {
    // ── পুরানো SW unregister করো, তারপর নতুনটা register করো ──
    // এটা নিশ্চিত করে পুরানো cache আর কাজ করবে না
    navigator.serviceWorker.getRegistrations().then(function(regs) {
      const unregisterAll = regs.map(r => r.unregister());
...
// ================================================================
// Firebase Config — MEMORY ONLY
// localStorage/sessionStorage ব্যবহার নেই।
// প্রতি page load / refresh-এ config মুছে যায়।
// প্রতিবার লগইনের আগে Firebase Config Modal পূরণ করতে হবে।
// ================================================================

// runtime-এ config একবার memory-তে রাখা হবে
let _runtimeFirebaseConfig = null;

function getFirebaseConfig() {
  return _runtimeFirebaseConfig; // localStorage নয় — শুধু memory
}

// database, auth, messaging শুরুতে undefined
let database, auth, messaging;

function _initFirebaseApp(cfg) {
  if (!firebase.apps.length) firebase.initializeApp(cfg);
  database = firebase.database();
  auth     = firebase.auth();
  try { messaging = firebase.messaging(); } catch(e) { messaging = null; }
  // auth null-guard listener চালু করো
  _setupAuthListener();
}

// ─── Page load হলে সবসময় config modal দেখাও ───
// FIX: script.js HTML-এর শেষে load হয়, তরবে DOMContentLoaded আগেই
// fire হয়ে যেতে পারে — readyState চেক দিয়ে নিরাপদভাবে handle করো
function _runOnDomReady(fn) {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', fn);
  } else {
    fn(); // DOM already ready — সাথে সাথে run করো
  }
}

_runOnDomReady(function() {
  // localStorage থেকে পুরানো config থাকলে মুছে দাও
  localStorage.removeItem('bisnu_fb_config');
  ...
  _showFirebaseConfigOnLoad();
});

// প্রতি page load-এ যে config modal দেখায়
function _showFirebaseConfigOnLoad() {
  ...
  if (h3) h3.innerHTML =
    '<i class="fas fa-fire" style="margin-right:8px;color:#ff8c00;"></i>' +
    'Firebase কনফিগ দিন';

  const hint = modal.querySelector('.firebase-config-hint');
  if (hint) hint.innerHTML =
    '<i class="fas fa-shield-alt" style="color:#00d4ff;margin-right:4px;"></i>' +
    'প্রতিবার লগইনের সময় Firebase config দিতে হবে। কোনো তথ্য সেভ হবে না।<br>' +
    '<i class="fas fa-info-circle" style="color:#00ff9d;margin-right:4px;"></i>' +
    'Firebase Console থেকে <code>const firebaseConfig = {...}</code> কপি করুন।';

  // current project section লুকাও
  const curSection = document.getElementById('firebase-current-project');
  if (curSection) curSection.style.display = 'none';

  // textarea পরিষ্কার রাখো
  const textarea = document.getElementById('firebaseConfigTextarea');
  if (textarea) textarea.value = '';

  // Cancel বাটন disable — config না দিলে এগোনো যাবে না
  const cancelBtn = modal.querySelector('.cancel-btn');
  if (cancelBtn) {
    cancelBtn.disabled = true;
    cancelBtn.style.opacity = '0.4';
    cancelBtn.style.cursor = 'not-allowed';
    cancelBtn.title = 'Firebase config না দিলে অ্যাপ চালু হবে না';
  }
}
...
// _setupAuthListener: _initFirebaseApp-এর পরে call হয়
function _setupAuthListener() {
  if (!auth) return;
  auth.onAuthStateChanged(function(user) {
      if (user) {
          database.ref('users/' + user.uid + '/operatingPassword').once('value', snap => {
              if (snap.exists() && snap.val()) correctOperatingPassword = snap.val();
          });
      }
...
function openFirebaseConfigModal() {
    // header-এ current project দেখাও যদি আগে দেওয়া থাকে
    const currentCfg = getFirebaseConfig();
    const dbEl = document.getElementById('firebase-current-db');
    if (dbEl) dbEl.innerText = currentCfg
        ? (currentCfg.databaseURL || currentCfg.projectId || 'সক্রিয় আছে')
        : 'এই session-এ দেওয়া হয়নি';
    document.getElementById('firebaseConfigError').innerText = '';
    document.getElementById('firebaseConfigTextarea').value = '';
    // currentCfg থাকলে Cancel চালু, না থাকলে বন্ধ
    const cancelBtn = document.querySelector('#firebaseConfigModal .cancel-btn');
    const curSection = document.getElementById('firebase-current-project');
    if (!currentCfg) {
        if (cancelBtn) { cancelBtn.disabled=true; cancelBtn.style.opacity='0.4'; cancelBtn.style.cursor='not-allowed'; }
        if (curSection) curSection.style.display='none';
    } else {
        if (cancelBtn) { cancelBtn.disabled=false; cancelBtn.style.opacity=''; cancelBtn.style.cursor=''; }
        if (curSection) curSection.style.display='';
    }
...
function saveFirebaseConfig() {
    ...
    try {
        ...
        // ════════════════════════════════════════════════════
        // ১৫: memory-তে সাভ করো — localStorage-এ নয়, refresh-এ মুছে যাবে
        _runtimeFirebaseConfig = cfg;

        // ১৬: Firebase সরাসরি init করো — reload লাগবে না
        try {
          _initFirebaseApp(cfg);
        } catch(initErr) {
          errorDiv.innerText = '❌ Firebase init সমস্যা: ' + initErr.message;
          _runtimeFirebaseConfig = null;
          return;
        }

        // ১৭: সফলতা — modal বন্ধ করে login screen দেখাও
        closeFirebaseConfigModal();
        const loginScreen = document.getElementById('login-screen');
        if (loginScreen) loginScreen.style.display = 'flex';
        showToast('✅ Firebase সঠিকভাবে সতেজা হয়েছে! লগইন করুন।', 'success');
...
document.addEventListener('click', function(event) {
  ...
  function deleteRule(id){
    const name=automationRules[id].name;
    promptForActionPassword('রুল মুছুন',`আপনি কি সত্যিই "${name}" রুলটি মুছে ফেলতে চান?`,()=>{
      // ✅ OPTIMISTIC UI
      const card=document.querySelector(`[data-rule-id="${id}"]`)||[...document.querySelectorAll('.rule-card')].find(c=>c.innerHTML.includes(name));
      if(card){card.style.transition='opacity 0.3s';card.style.opacity='0';setTimeout(()=>card.remove(),300);}
      delete automationRules[id];
      database.ref(`${getPathFor('automation_rules')}/${id}`).remove()
        .then(()=>{showToast(`✅ রুল '${name}' মুছে ফেলা হয়েছে।`,'success');logAuditEvent(`'${name}' রুলটি মুছে ফেলেছেন।`);})
        .catch(err=>{showToast(`❌ মুছতে সমস্যা: ${err.message}`,'error');setTimeout(()=>location.reload(),1500);});
    });
  }
  ...
  function deleteScene(id,name){
    promptForActionPassword('দৃশ্য মুছুন',`আপনি কি "${name}" দৃশ্যটি স্থায়ীভাবে মুছে ফেলতে চান?`,()=>{
      // ✅ OPTIMISTIC UI — scene button container সরাও
      const sceneContainers=[...document.querySelectorAll('.scene-item-container')].find(c=>c.innerHTML.includes(name));
      if(sceneContainers){sceneContainers.style.transition='opacity 0.3s';sceneContainers.style.opacity='0';setTimeout(()=>sceneContainers.remove(),300);}
      database.ref(`${getPathFor('scenes')}/${id}`).remove()
        .then(()=>{showToast(`✅ দৃশ্য '${name}' মুছে ফেলা হয়েছে।`,'success');logAuditEvent(`দৃশ্য '${name}' মুছে ফেলেছেন।`);})
        .catch(err=>{showToast(`❌ মুছতে সমস্যা: ${err.message}`,'error');setTimeout(()=>location.reload(),1500);});
    });
  }
  ...
  function renderControlPanel(devsData, relNames) {
      ...
      // ✅ BUG FIX: মুছে যাওয়া device-এর পুরানো section DOM থেকে সরাও
      Array.from(cont.querySelectorAll('.control-section[id^="section-"]')).forEach(section => {
          const sectionDevId = section.id.replace('section-', '');
          if (!devsData.hasOwnProperty(sectionDevId)) {
              section.remove();
          }
      });
  ...
  function deleteDevice(devId){
    promptForActionPassword('ডিভাইস মুছুন',`আপনি কি সত্যিই '${devId}' এবং এর সমস্ত রিলে মুছে ফেলতে চান?`,()=>{
      // ✅ OPTIMISTIC UI: Firebase-এর response আসার আগেই DOM থেকে সরাও
      const section = document.getElementById(`section-${devId}`);
      if (section) {
      ...
      // Local state থেকেও সরাও
      if (currentDeviceStates) delete currentDeviceStates[devId];
      if (deviceMetadata) delete deviceMetadata[devId];
      if (relayNames) delete relayNames[devId];

      const updates={};
      updates[`${getPathFor('devices_root')}/${devId}`]=null;
      updates[`${getPathFor('device_metadata')}/${devId}`]=null;
      database.ref(getPathFor('timers')).orderByKey().startAt(devId).endAt(devId+'\uf8ff').once('value',snap=>{
        const timers=snap.val();
        if(timers) for(const key in timers) updates[`${getPathFor('timers')}/${key}`]=null;
        database.ref().update(updates)
          .then(()=>{
            showToast(`✅ ডিভাইস '${devId}' সফলভাবে মুছে ফেলা হয়েছে।`, 'success');
            logAuditEvent(`ডিভাইস '${devId}' মুছে ফেলেছেন।`);
          })
          .catch(err=>{
            // ব্যর্থ হলে পেজ রিলোড করে সঠিক অবস্থায় ফিরিয়ে আনো
            showToast(`❌ মুছতে সমস্যা হয়েছে: ${err.message}`, 'error');
            setTimeout(() => location.reload(), 1500);
          });
      });
    });
  }
  function deleteRelay(devId,relId){
    const name=(relayNames[devId]?.[relId])||relId;
    promptForActionPassword('রিলে/লোড মুছুন',`আপনি কি সত্যিই '${name}' রিলে/লোডটি '${devId}' থেকে মুছে ফেলতে চান?`,()=>{
      // ✅ OPTIMISTIC UI: DOM থেকে card এবং toggle-switch সরাও
      const card = document.getElementById(`card-${devId}-${relId}`);
      if (card) {
      ...
      if (currentDeviceStates[devId]) delete currentDeviceStates[devId][relId];
      if (relayNames[devId]) delete relayNames[devId][relId];

      const updates={};
      updates[`${getPathFor('devices_root')}/${devId}/${relId}`]=null;
      updates[`${getPathFor('device_metadata')}/${devId}/relay_names/${relId}`]=null;
      updates[`${getPathFor('timers')}/${devId}-${relId}`]=null;
      database.ref().update(updates)
        .then(()=>{
          showToast(`✅ '${name}' সফলভাবে মুছে ফেলা হয়েছে।`, 'success');
          logAuditEvent(`'${devId}' থেকে '${name}' মুছে ফেলেছেন।`);
        })
        .catch(err=>{
          showToast(`❌ মুছতে সমস্যা হয়েছে: ${err.message}`, 'error');
          setTimeout(() => location.reload(), 1500);
  ...
  function deleteTemperatureSensor(id,name){
    promptForActionPassword('সেন্সর গ্রুপ মুছুন',`আপনি কি সত্যিই '${name}' সেন্সর গ্রুপটি মুছে ফেলতে চান?`,()=>{
      // ✅ OPTIMISTIC UI
      const cont=document.getElementById('temperature-sensors-container');
      if(cont){const cards=[...cont.querySelectorAll('.temp-sensor-card')].find(c=>c.innerHTML.includes(name)||c.innerHTML.includes(id));if(cards){cards.style.transition='opacity 0.3s';cards.style.opacity='0';setTimeout(()=>cards.remove(),300);}}
      if(sensorHistoricalCharts[id]){try{sensorHistoricalCharts[id].destroy();}catch(e){}delete sensorHistoricalCharts[id];}
      delete currentTempData[id];
      if(deviceMetadata) delete deviceMetadata[id];
      const updates={};
      updates[`${getPathFor('devices_root')}/${id}`]=null;
      updates[`${getPathFor('device_metadata')}/${id}`]=null;
      database.ref().update(updates)
        .then(()=>{showToast(`✅ '${name}' সেন্সর মুছে ফেলা হয়েছে।`,'success');logAuditEvent(`তাপমাত্রা সেন্সর গ্রুপ '${name}' মুছে ফেলেছেন।`);})
        .catch(err=>{showToast(`❌ মুছতে সমস্যা: ${err.message}`,'error');setTimeout(()=>location.reload(),1500);});
    });
  }
  ...
  function deleteCustomPage(id,name,e){
    e.stopPropagation();
    promptForActionPassword('পেজ মুছুন',`আপনি কি সত্যিই '${name}' পেজটি মুছে ফেলতে চান?`,()=>{
      // ✅ OPTIMISTIC UI — nav tab ও page content সরাও
      const navTab=document.querySelector(`.nav-tab[data-tab-id="custom_page_${id}"]`);
      const pageEl=document.getElementById(`custom_page_${id}`);
      if(navTab){navTab.style.transition='opacity 0.3s';navTab.style.opacity='0';setTimeout(()=>navTab.remove(),300);}
      if(pageEl) pageEl.remove();
      showPage('page1');
      database.ref(`${getPathFor('custom_pages')}/${id}`).remove()
        .then(()=>{showToast(`✅ পেজ '${name}' মুছে ফেলা হয়েছে।`,'success');logAuditEvent(`কাস্টম পেজ '${name}' মুছে ফেলেছেন।`);})
        .catch(err=>{showToast(`❌ মুছতে সমস্যা: ${err.message}`,'error');setTimeout(()=>location.reload(),1500);});
    });
  }
  ...
  function renderDynamicPageContent(cont,comps){
    ...
    function deleteLogicGateRule(id) {
      const name = logicGateRules[id].name;
      promptForActionPassword('লজিক গেট রুল মুছুন', `আপনি কি "${name}" রুলটি মুছে ফেলতে চান?`, () => {
        // ✅ OPTIMISTIC UI
        const card=[...document.querySelectorAll('.rule-card')].find(c=>c.innerHTML.includes(name));
        if(card){card.style.transition='opacity 0.3s';card.style.opacity='0';setTimeout(()=>card.remove(),300);}
        delete logicGateRules[id];
        database.ref(`${getPathFor('logic_gate_rules')}/${id}`).remove()
          .then(()=>showToast(`✅ লজিক রুল '${name}' মুছে ফেলা হয়েছে।`,'success'))
          .catch(err=>{showToast(`❌ মুছতে সমস্যা: ${err.message}`,'error');setTimeout(()=>location.reload(),1500);});
      });
    }
