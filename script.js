/* ================================================================
   BISNU IoT ডিভাইস কন্ট্রোল প্যানেল — script.js
================================================================ */
...
document.addEventListener('click', function(event) {
  ...
  function updateEnergyData(data) {
    ...
    const setHTML = (id, val, unit) => {
      const el = document.getElementById(id);
      if (!el) return;
      el.innerHTML = (data[val] !== undefined && data[val] !== null) ? data[val] + ` <span class="energy-unit">${unit}</span>` : `-- <span class="energy-unit">${unit}</span>`;
    };
    const setText = (id, val) => {
      const el = document.getElementById(id);
      if (el) el.textContent = (data[val] !== undefined && data[val] !== null) ? data[val] : '--';
    };
    ...
    // পিক পাওয়ার ও আজকের এনার্জি
    setHTML('peak-power-value', 'peak_power', 'W');
    setHTML('today-energy-value', 'today_energy', 'kWh');
    ...
    // ===== রিলে স্ট্যাটাস =====
    const relayColor = (val) => {
      if (val === 'ON' || val === true) return 'color: var(--success-color); font-weight:700;';
      if (val === 'OFF' || val === false) return 'color: var(--accent-color); font-weight:700;';
      return '';
    };
    const protColor = (val) => val === true ? 'color: var(--warning-color); font-weight:700;' : 'color: var(--text-dim);';

    const setRelay = (elId, val) => {
      const el = document.getElementById(elId);
      if (!el) return;
      const display = (val !== undefined && val !== null) ? String(val).toUpperCase() : '--';
      el.innerHTML = `<span style="${relayColor(val)}">${display}</span>`;
    };
    const setProt = (elId, val) => {
      const el = document.getElementById(elId);
      if (!el) return;
      const display = (val !== undefined && val !== null) ? (val === true ? '⚠️ সক্রিয়' : '✅ নিষ্ক্রিয়') : '--';
      el.innerHTML = `<span style="${protColor(val)}">${display}</span>`;
    };

    setRelay('relay-status-value', data.relay_status);
    setProt('protection-triggered-value', data.protection_triggered);
    setRelay('relay1-status-value', data.relay1_status);
    setProt('relay1-protection-value', data.relay1_protection);
    setRelay('relay2-status-value', data.relay2_status);
    setProt('relay2-protection-value', data.relay2_protection);
    setRelay('relay3-status-value', data.relay3_status);
    setProt('relay3-protection-value', data.relay3_protection);
    setRelay('relay4-status-value', data.relay4_status);
    setProt('relay4-protection-value', data.relay4_protection);

    // ===== ক্যালিব্রেশন =====
    setText('calib-v-mult-value', 'calib_v_mult');
    setText('calib-v-offset-value', 'calib_v_offset');
    setText('calib-a-mult-value', 'calib_a_mult');
    setText('calib-a-offset-value', 'calib_a_offset');
