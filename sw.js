...
const CACHE_NAME = 'bisnu-iot-v2';
const RUNTIME_CACHE = 'bisnu-runtime-v2';

// ── IMPORTANT: script.js ও index.html PRECACHE থেকে বাদ ──
// Firebase config memory-only হওয়ায় এগুলো সবসময় network থেকে আসতে হবে।
// Cache হলে পুরানো hardcoded-config ফাইল serve হতে পারে।
...
// ── এই ফাইলগুলো সবসময় network-first (কখনো cache করবে না) ──
const NEVER_CACHE = [
  './index.html',
  './',
  './script.js',
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.origin === self.location.origin) {
    // script.js ও index.html কখনো cache করবে না
    const neverCache = NEVER_CACHE.some(p =>
      url.pathname.endsWith(p.replace('./', '/')) || url.pathname === p.replace('.', '')
    );
    if (neverCache) {
      event.respondWith(fetch(request)); // সরাসরি network থেকে নাও
      return;
    }
